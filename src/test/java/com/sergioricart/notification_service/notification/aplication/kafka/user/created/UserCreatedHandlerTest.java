...
class UserCreatedHandlerTest {
    ...
    void handle_givenValidCommand_sendsEmailWithCorrectArguments() {
        handler.handle(NotificationFixture.aUserCreatedCommand());

        verify(notificationPort).sendUserCreatedEmail(
    ...
    void handle_givenValidCommand_returnsVoidResponse() {
        VoidResponse result = handler.handle(NotificationFixture.aUserCreatedCommand());

        assertThat(result).isNotNull();
    }

    @Test
    void getCommandType_returnsUserCreatedCommandClass() {
        assertThat(handler.getCommandType()).isEqualTo(UserCreatedCommand.class);
    }
}
