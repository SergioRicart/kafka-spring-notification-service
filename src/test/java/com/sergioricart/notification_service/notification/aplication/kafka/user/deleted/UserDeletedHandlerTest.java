...
class UserDeletedHandlerTest {
    ...
    @InjectMocks
    private UserDeletedHandler handler;

    @Test
    void handle_givenValidCommand_sendsEmailWithCorrectArguments() {
        handler.handle(NotificationFixture.aUserDeletedCommand());

        verify(notificationPort).sendUserDeletedEmail(
    ...
    void handle_givenValidCommand_returnsVoidResponse() {
        VoidResponse result = handler.handle(NotificationFixture.aUserDeletedCommand());

        assertThat(result).isNotNull();
    }

    @Test
    void getCommandType_returnsUserDeletedCommandClass() {
        assertThat(handler.getCommandType()).isEqualTo(UserDeletedCommand.class);
    }
}
