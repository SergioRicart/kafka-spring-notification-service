...
class UserUpdatedHandlerTest {
    ...
    @InjectMocks
    private UserUpdatedHandler handler;

    @Test
    void handle_givenValidCommand_sendsEmailWithCorrectArguments() {
        handler.handle(NotificationFixture.aUserUpdatedCommand());

        verify(notificationPort).sendUserUpdatedEmail(
    ...
    void handle_givenValidCommand_returnsVoidResponse() {
        VoidResponse result = handler.handle(NotificationFixture.aUserUpdatedCommand());

        assertThat(result).isNotNull();
    }

    @Test
    void getCommandType_returnsUserUpdatedCommandClass() {
        assertThat(handler.getCommandType()).isEqualTo(UserUpdatedCommand.class);
    }
}
